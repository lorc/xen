...
struct vchan_client {
    LIBXL_LIST_ENTRY(struct vchan_client) list;
...
static LIBXL_LIST_HEAD(clients_list, struct vchan_client) vchan_clients;
                            ...
                            struct clients_list *list, char **watch_ret)
{
    ...
    pthread_mutex_lock(&vchan_client_mutex);
    LIBXL_LIST_INSERT_HEAD(&vchan_clients, new_client, list);
    pthread_mutex_unlock(&vchan_client_mutex);
}
...
static void terminate_clients(void)
{
    ...
    pthread_mutex_lock(&vchan_client_mutex);
    LIBXL_LIST_FOREACH(client, &vchan_clients, list) {
