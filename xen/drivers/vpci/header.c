    ...
    unsigned long s, unsigned long e, void *data, unsigned long *c)
{
    ...
    for ( ; ; )
    {
        ...
        if ( !iomem_access_permitted(map->d, mfn_x(map_mfn), e) )
        {
        ...
        rc = xsm_iomem_mapping(XSM_HOOK, map->d, mfn_x(map_mfn), e, map->map);
        if ( rc )
        {
            printk(XENLOG_G_WARNING
                            ...
                            bool rom_only)
{
    ...
    if ( !rom_only )
    {
        ...
        header->guest_cmd &= ~PCI_COMMAND_MEMORY | (cmd & PCI_COMMAND_MEMORY);
